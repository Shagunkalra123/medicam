    ...
    "scrollX": true,
       ...
       "searching": true,
		...
		columns: [
          { data: 'id',render: function ( data, type, row ) {
			if(data != null){
				return '<span data-product_id="'+row.id+'" >'+data+'</span>';
			}
			return '<span data-product_id="'+row.id+'"></span>';
		  },orderable:true},
		  ...
		  { data: 'meeting_with',render: function ( data, type, row ) {
			if(data != null){
				return '<span data-product_id="'+row.id+'" >'+data+'</span>';
			}
			return '<span data-product_id="'+row.id+'"></span>';
		  },orderable:false},
		  { data: 'meeting_date',render: function ( data, type, row ) {
			if(data != null){
				return '<span data-product_id="'+row.id+'" >'+data+'</span>';
			}
			return '<span data-product_id="'+row.id+'"></span>';
		  ...
		  { data: 'assgin_to',render: function ( data, type, row ) {
			if(data != null){
				return '<span data-product_id="'+row.id+'" >'+data+'</span>';
			}
			return '<span data-product_id="'+row.id+'"></span>';
		  ...
		  @can('isAdmin')
		  { data: 'id',render: function ( data, type, row ) {
			return '<button class="btn btn-success editproductModal" type="button" onclick="editProduct(this)" name="edit" data-product_id="'+row.id+'"> <i class="fas fa-edit"></i></button><button class="btn btn-danger deleteproductModal" name="edit" data-product_id="'+row.id+'"> <i class="fas fa-trash-alt"></i></button>';
		  },orderable:false},
		  @endcan
		 ],
     ...
     "initComplete": function(settings, json) {
      $("#data_table td, #data_table1 td").each(function(i,v){
          let id = $(this).find('span').data('product_id');
          if(i < 8){
            $(this).attr('onclick','editProduct(this)');
            $(this).attr('data-product_id',id); 
          }
      });
    }
    });
...
$('#createNewProduct').click(function () {
    ...
    function editProduct(pointer){
        var meeting_id=$(pointer).attr("data-product_id");
